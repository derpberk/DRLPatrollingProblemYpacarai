...
class DuelingDQNAgent:
    ...
    def __init__(
            ...
            tau: float = 0.0001,
            epsilon_values: Tuple[float, float] = (1.0, 0.0),
            epsilon_interval: Tuple[float, float] = (0.0, 1.0),
    ...
    def train(self, episodes):
        ...
        for episode in range(1, int(episodes) + 1):
            ...
            if self.save_every != 0 and self.episode % self.save_every == 0:
                print(f"Policy saved at Episode {self.episode}!")
                self.save_model(name=f'Policy_Episode_{self.episode}.pth')
