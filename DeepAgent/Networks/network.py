...
class NoisyVisualNetwork(nn.Module):
	def __init__(
			self,
			in_dim: tuple,
			out_dim: int,
		...
		"""Initialization."""
		super(NoisyVisualNetwork, self).__init__()
		...
		# set common feature layer
		self.feature_layer = FeatureExtractor(in_dim, number_of_features)

		self.hidden_1 = NoisyLinear(number_of_features, 256)
		self.hidden_2 = NoisyLinear(256, 256)
		self.hidden_3 = NoisyLinear(256, 256)
	...
	def dist(self, x: torch.Tensor) -> torch.Tensor:
		...
		feature = F.relu(self.hidden_1(feature))
		feature = F.relu(self.hidden_2(feature))
		feature = F.relu(self.hidden_3(feature))

		# Compute hidden adv
		adv_hid = F.relu(self.advantage_hidden_layer(feature))

		# Compute hidden value
		val_hid = F.relu(self.value_hidden_layer(feature))
		...
		dist = F.softmax(q_atoms, dim=-1)
		dist = dist.clamp(min=1e-5)  # for avoiding nans
	...
	def reset_noise(self):
		...
		self.hidden_1.reset_noise()
		self.hidden_2.reset_noise()
		self.hidden_3.reset_noise()
...
class VisualNetwork(nn.Module):
	def __init__(
		...
		self.advantage_layer = nn.Linear(64, out_dim * atom_size)

		# set value layer
		self.value_hidden_layer = nn.Linear(256, 64)
		self.value_layer = nn.Linear(64, atom_size)
